...
use, caching them at ~/.cache/huggingface/hub/ by default. The tiny
multilingual int8 model is used by default.
...
runtime never needs network. Bundling the model inside the installer is
...
def _resolve_model_id(model_size: str, language: str | None) -> str:
    ...
    them when language is explicitly set to 'en'.

    When language is None, the multilingual model is used so that
    faster-whisper can automatically detect the spoken language.
    """
...
class FasterWhisperSTT:
    ...
    async def load(self) -> None:
        ...
        try:
            ...
            #
            # Keep download_root in the constructor arguments even when
            # it is None. The STT contract/tests expect the parameter to
            # be explicit in both configured and unconfigured cases.
            kwargs: dict = {
                "device": self._device,
                "compute_type": self.compute_type,
                "download_root": (
                    str(self._download_root)
    ...
    def _sync_transcribe(self, audio: np.ndarray) -> str:
        ...
        Always called inside asyncio.to_thread; never on the loop thread.
        """
        assert self._model is not None

        # language=None enables automatic language detection.
        # When an explicit language such as "en" is configured, that
        # language is passed through and decoding is constrained accordingly.
